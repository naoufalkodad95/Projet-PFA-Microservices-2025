// src/components/challenges/NewChallengeModal.jsx
...
const NewChallengeModal = ({ onClose, onSubmit, teams = [], currentTeamId }) => {
  // États du formulaire
  const [formData, setFormData] = useState({
    equipeAdverseId: '',
    dateDefi: getTodayDate(),
  ...
  const checkTerrainAvailability = async () => {
    if (!formData.terrain || !formData.dateDefi || !formData.heureDebut || !formData.heureFin) {
    ...
    try {
      ...
      const dateDebut = new Date(`${formData.dateDefi}T${formData.heureDebut}`);
      const dateFin = new Date(`${formData.dateDefi}T${formData.heureFin}`);
  ...
  const validateForm = async () => {
    ...
    if (!formData.dateDefi) {
      newErrors.dateDefi = 'Veuillez sélectionner une date';
    }
  ...
  const handleSubmit = async () => {
    ...
    if (isValid) {
      try {
        ...
        // Appel à l'API pour créer le défi
        await defiService.createDefi(currentTeamId, defiData);
        ...
        setIsSubmitting(false);
        setErrors(prev => ({ ...prev, submit: 'Erreur lors de la création du défi. Veuillez réessayer.' }));
      }
    }
  };
  ...
  return (
      <ModalOverlay>
        <ModalContainer>
          <ModalHeader>
            <ModalTitle>Nouveau défi</ModalTitle>
          ...
          <ModalBody>
            ...
            <FormGroup>
              <Label htmlFor="dateDefi">Date</Label>
              <Input
                  type="date"
                  id="dateDefi"
                  name="dateDefi"
                  ...
                  min={getTodayDate()}
              />
              {errors.dateDefi && <ErrorMessage>{errors.dateDefi}</ErrorMessage>}
            </FormGroup>
            ...
            <FormGroup>
              ...
              <TextArea
                  ...
                  onChange={handleChange}
                  placeholder="Ajouter un message pour l'équipe adverse..."
                  maxLength={500}
              />
            </FormGroup>
          ...
          <ModalFooter>
            ...
            <Button
                ...
                disabled={isSubmitting}
            >
              {isSubmitting ? 'Envoi en cours...' : 'Envoyer le défi'}
            </Button>
          </ModalFooter>
        </ModalContainer>
      </ModalOverlay>
